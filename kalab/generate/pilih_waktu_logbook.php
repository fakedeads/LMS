...
$idcourse = strip_tags(mysql_escape_string($_GET['id']));
$dataLogbook=mysql_fetch_array(mysql_query("SELECT * FROM  tb_list_logbook  WHERE id ='$idpenilai'"));
$judulLOgbook=$dataLogbook['judulLogbook'];
$idcategori=$dataLogbook['idcategori'];
$idcourse=$dataLogbook['idcourse'];
$modul=$dataLogbook['modul'];
?>
	...
	<div style="margin: 0 auto; width: 672px; background-color: #ffffff; border-radius: 15px; padding-top: 3px; padding-bottom: 7px; margin-bottom: 13px; margin-top: 13px;">
		...
		<table width="600">
			...
			<tr>
				...
				?> / 
				<?php
					$quer=mysql_fetch_array(mysql_query("SELECT * FROM tb_list_logbook WHERE idcourse = '$idcourse'")); 
					echo $quer ['modul'];
				?>
				</td>
			</tr>
		...
		<table class="datatable" border="1" align="center">
			...
			<tr>
				...
				<td align="center">
				<a href="kordas/generate/pdfLogbook.php?idlogbook=<?echo $idlogbook;?>&date=<?php echo $data['date'];?>&time=<?php echo $data['waktu'];?>" target="_blank">
				<img src="../../gambar/menu/zoom4.png" width="22" height="22" /> </a></td>
	<?
	}
	?>
			</tr>
		</table>
		<span style="font-size: 14px;"><a href="course.php?idcategori=1">Back</a></span>
	</div>
