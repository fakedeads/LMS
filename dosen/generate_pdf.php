	...
	private $options = array(
    	...
    	$this->options = $options;
  		...
  		public function rptDetailData () {
  			//
  			$idcategori=$_GET['kd_mk'];
  			$categoriCourse=mysql_query("SELECT * FROM   tb_matakuliah WHERE kd_mk ='$idcategori'");
  			$showDate=mysql_fetch_array($categoriCourse);

  			$sqlBanyakModul=mysql_query("SELECT * FROM tb_mdl_praktikum WHERE kd_mk=$idcategori");
  			$sqlBanyakModul1=mysql_query("SELECT * FROM tb_mdl_praktikum WHERE kd_mk=$idcategori");
  			...
  			$this->Ln(10);
  			$this->SetFont("", "B", 12);
  			$this->SetX($left); $this->Cell(0, 10, $showDate['kd_mk'].' '.$showDate['nama_mk'], 0, 1,'C');
  			$this->Ln(10);
  			$this->SetFont("", "B", 12);
...
$data = array();
$dateCategori1=$showDate['kd_mk'];
$sqlMahasiswa1=mysql_query("SELECT nim FROM  tb_jadwal_praktikum WHERE kd_mk ='$dateCategori1'");
while ($mahasiswa1=mysql_fetch_array($sqlMahasiswa1))
{
	$idMahasiswa1=$mahasiswa1['nim'];
	$sqlketMahasiswa1=mysql_query("SELECT * FROM  tb_mahasiswa WHERE nim ='$idMahasiswa1'");
...
$sqlMahasiswa=mysql_query("SELECT nim FROM  tb_jadwal_praktikum WHERE kd_mk ='$idcategori'");
while ($mahasiswa=mysql_fetch_array($sqlMahasiswa))
{
	...
	$sqlModul=mysql_query("SELECT * FROM tb_mdl_praktikum WHERE kd_mk='$idcategori'");
	...
	while ($modul=mysql_fetch_array($sqlModul))
	{
		...
		//echo $idcourse;
		$sqlNilaiAwal=mysql_query("SELECT * FROM tb_idpenilaianpelaksananpraktikum WHERE kd_mk='$idcategori' AND kd_modul='$idcourse'");
		$total=0;
		$nilaimodul=0;
